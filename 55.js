  ...
  async queryPrizeList() {
    ...
    for (let retry = 0; retry < 3; retry++) {
      try {
        ...
        this.debug(buildApiDebugLine("prizeList", data));
        const summary = buildPacketSummary(data, "商品列表接口未返回提示");

        console.log("=".repeat(60));
        console.log(
          `[3] 商品列表结果：code=${summary.code || "-"} | msg=${summary.message || "-"} | 上午场${Array.isArray(data?.data?.secKillPrizeAM) ? data.data.secKillPrizeAM.length : 0}项 | 下午场${Array.isArray(data?.data?.secKillPrizePM) ? data.data.secKillPrizePM.length : 0}项`
        );
    ...
    this.error(`查询商品失败（已重试3次）：${message}`);
