...
class SeckillService {
  ...
  getConfig() {
    const CONFIG = {
      ...
      user_agent: "Mozilla/5.0 (iPhone; CPU iPhone OS 18_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko)  unicom{version:iphone_c@12.1001};ltst;OSVersion/18.5",
      ...
      cookie: "br-session-cache-e4466c71aafc4b578efcde9a51971345=[{\"appId\":\"e4466c71aafc4b578efcde9a51971345\",\"sessionID\":\"2b693183-496c-4b3d-ac97-8d292fcbdc22\",\"lastVisitedTime\":1775530832115,\"startTime\":0,\"isRestSID\":false}]; _pk_id.3.56f6=929217b66ee11f90.1775527377." // 可从环境变量获取
    };
  ...
  async checkSCUser() {
    ...
    try {
      const ts = Date.now();
      const url = `https://${cfg.headers["Host"]}/2b2c-mobile/api/seckill/checkSCUser`;
      const params = {
      ...
      const { data } = await axios.get(url, { headers: cfg.headers, params });
      this.debug(`checkSCUser响应: ${JSON.stringify(data)}`);
      console.log("=".repeat(50));
      console.log("[1] 资格校验结果：", data);

      return data.resultCode === "0000" && data.data === true;
    } catch (e) {
  ...
  async checkUser() {
    ...
    try {
      const ts = Date.now();
      const url = `https://${cfg.headers["Host"]}/2b2c-mobile/api/seckill/checkUser`;
      const params = {
        phoneNumber: cfg.phoneNumber,
        activityId: cfg.activityId,
        timestamp: ts
      ...
      console.log("=".repeat(50));
      console.log("[2] 参与状态校验结果：", data);

      return data.resultCode === "0000" && data.data === false;
    } catch (e) {
  ...
  async queryPrizeList() {
    ...
    try {
      const ts = Date.now();
      const url = `https://${cfg.headers["Host"]}/2b2c-mobile/api/seckill/prizeList`;
      const params = {
        activityId: cfg.activityId,
        timestamp: ts,
        phoneNumber: cfg.phoneNumber
