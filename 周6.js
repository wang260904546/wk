...
const AFTERNOON_TARGETS = [
  { id: "11", name: "爱奇艺黄金VIP会员-月卡" },
  { id: "12", name: "哔哩哔哩大会员" },
  { id: "13", name: "10元话费券" },
  { id: "14", name: "滴滴快车5元代金券" }
];
...
async function main() {
  ...
  // 发送总结果通知
  const successServices = services.filter(service => service.eligible);
  if (successServices.length > 0) {
    const currentTime = new Date().toLocaleString('zh-CN');
    let pushContent = `📣 【四川联通周二福利秒杀总结果】\n\n📊 总计执行 ${services.length} 个账号\n\n`;
    
    successServices.forEach(service => {
      pushContent += `账号[${service.index}] 📱 手机号：${service.getConfig().phoneNumber} 执行状态：${service.eligible ? "成功" : "失败"}\n`;
    });
    
    pushContent += `\n✨ 抢购任务完成！`;
    
    // 使用第一个服务发送总结果
    await successServices[0].sendWechatMessage(pushContent);
  }
}
